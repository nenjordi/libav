...
#define MAX_DELAY            5         /* limit for main profile for frame
...
typedef struct DiracContext {
    ...
    uint16_t *mctmp;            /* buffer holding the MC data multipled by
                                   OBMC weights */
    uint8_t *mcscratch;

    DECLARE_ALIGNED(16, uint8_t, obmc_weight)[3][MAX_BLOCKSIZE*MAX_BLOCKSIZE];

    void (*put_pixels_tab[4])(uint8_t *dst, const uint8_t *src[5],
                              int stride, int h);
    void (*avg_pixels_tab[4])(uint8_t *dst, const uint8_t *src[5],
                              int stride, int h);
...
static int alloc_sequence_buffers(DiracContext *s)
{
    ...
    s->mctmp     = av_malloc((w + 64 + MAX_BLOCKSIZE) * (h * MAX_BLOCKSIZE)
                             * sizeof(*s->mctmp));
                                                     ...
                                                     SubBand *b, int is_arith)
{
    ...
    DiracArith c;
    GetBitContext gb;
    int cb_width  = s->codeblock[b->level +
                                 (b->orientation != subband_ll)].width;
    int cb_height = s->codeblock[b->level +
                                 (b->orientation != subband_ll)].height;
    ...
    for (slice_y = 0; bufsize > 0 && slice_y < s->lowdelay.num_y; slice_y++)
        for (slice_x = 0; bufsize > 0 && slice_x < s->lowdelay.num_x; slice_x++) {
            bytes = (slice_num + 1) * s->lowdelay.bytes.num
                / s->lowdelay.bytes.den - slice_num * s->lowdelay.bytes.num
                                 ...
                                 int width, int height)
{
    ...
    s->dsp.draw_edges(ref->hpel[plane][0], ref->avframe.linesize[plane],
                      width, height, edge, edge, EDGE_TOP | EDGE_BOTTOM);
    /* WARNING: EDGE_TOP | EDGE_BOTTOM values just copied to make it build,
    ...
    if (!ref->interpolated[plane]) {
        s->diracdsp.dirac_hpel_filter(ref->hpel[plane][1], ref->hpel[plane][2],
                                      ...
                                      height);
        s->dsp.draw_edges(ref->hpel[plane][1], ref->avframe.linesize[plane],
                          width, height, edge, edge, EDGE_TOP | EDGE_BOTTOM);
        s->dsp.draw_edges(ref->hpel[plane][2], ref->avframe.linesize[plane],
                          width, height, edge, edge, EDGE_TOP | EDGE_BOTTOM);
        s->dsp.draw_edges(ref->hpel[plane][3], ref->avframe.linesize[plane],
                          width, height, edge, edge, EDGE_TOP | EDGE_BOTTOM);
    }
...
static int dirac_decode_frame_internal(DiracContext *s)
{
    ...
    for (comp = 0; comp < 3; comp++) {
        ...
        if (ff_spatial_idwt_init2(&d, p->idwt_buf, p->idwt_width,
...
/* [DIRAC_STD] dirac_decode_data_unit makes reference
   to the while defined in 9.3
