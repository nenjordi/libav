...
/* The spec limits this to 3 for frame coding, but in practice it can be as
 * high as 6. */
#define DIRAC_MAX_REFERENCE_FRAMES 8
#define DIRAC_MAX_DELAY            5   /* limit for main profile for frame
                                        * coding (TODO: field coding) */
#define DIRAC_MAX_FRAMES           (DIRAC_MAX_REFERENCE_FRAMES + DIRAC_MAX_DELAY + 1)
#define DIRAC_MAX_QUANT            68  /* max quant for VC-2 */
#define DIRAC_MAX_BLOCKSIZE        32  /* maximum xblen/yblen we support */
...
typedef struct DiracPlane {
    ...
    DiracSubBand band[DIRAC_MAX_DWT_LEVELS][4];
} DiracPlane;
...
typedef struct DiracContext {
    ...
    /* Schroedinger older than 1.0.8 doesn't store quant
     * delta if only one codebook exists in a band. */
    ...
    struct codeblock {
        unsigned width;
        unsigned height;
    } codeblock[DIRAC_MAX_DWT_LEVELS + 1];

    struct lowdelay {
        unsigned num_x;         /* number of horizontal slices               */
        unsigned num_y;         /* number of vertical slices                 */
        AVRational bytes;       /* average bytes per slice                   */
        uint8_t quant[DIRAC_MAX_DWT_LEVELS][4]; /* [DIRAC_STD] E.1 */
    } lowdelay;
    ...
    uint16_t *mctmp;            /* buffer holding the MC data multiplied by
                                 * OBMC weights */
    uint8_t *mcscratch;

    DECLARE_ALIGNED(16, uint8_t, obmc_weight)[3][DIRAC_MAX_BLOCKSIZE * DIRAC_MAX_BLOCKSIZE];

    void (*put_pixels_tab[4])(DiracDSPContext *dc, uint8_t *dst,
                              const uint8_t *src[5], int stride, int h);
    void (*avg_pixels_tab[4])(DiracDSPContext *dc, uint8_t *dst,
                              const uint8_t *src[5], int stride, int h);
    void (*add_obmc)(uint16_t *dst, const uint8_t *src, int stride,
    ...
    DiracFrame *ref_frames[DIRAC_MAX_REFERENCE_FRAMES + 1];
    DiracFrame *delay_frames[DIRAC_MAX_DELAY + 1];
    DiracFrame all_frames[DIRAC_MAX_FRAMES];
} DiracContext;
...
static const int qscale_tab[DIRAC_MAX_QUANT + 1] = {
        4,     5,     6,     7,     8,    10,    11,    13,
...
static const int qoffset_intra_tab[DIRAC_MAX_QUANT + 1] = {
        1,     2,    3,     4,     4,     5,     6,      7,
...
static const int qoffset_inter_tab[DIRAC_MAX_QUANT + 1] = {
        1,     2,    2,    3,     3,     4,     4,      5,
...
static int alloc_sequence_buffers(DiracContext *s)
{
    ...
    /* TODO: think more about this / use or set DiracPlane here */
    for (i = 0; i < 3; i++) {
        int max_xblen = DIRAC_MAX_BLOCKSIZE >> (i ? s->chroma_x_shift : 0);
        int max_yblen = DIRAC_MAX_BLOCKSIZE >> (i ? s->chroma_y_shift : 0);
        w = s->source.width  >> (i ? s->chroma_x_shift : 0);
        h = s->source.height >> (i ? s->chroma_y_shift : 0);

        /* We allocate the max we support here since num decompositions can
         * change from frame to frame. Stride is aligned to 16 for SIMD, and
         * 1<<DIRAC_MAX_DWT_LEVELS top padding to avoid if (y>0) in arith
         * decoding DIRAC_MAX_BLOCKSIZE padding for MC: blocks can spill up
         * to half of that on each side. */
        top_padding = FFMAX(1 << DIRAC_MAX_DWT_LEVELS, max_yblen / 2);
        w           = FFALIGN(CALC_PADDING(w, DIRAC_MAX_DWT_LEVELS), 8);
        /* FIXME: Should w align be 16 for SSE? */
        h           = top_padding + CALC_PADDING(h, DIRAC_MAX_DWT_LEVELS) +
                      max_yblen / 2;

        s->plane[i].idwt_buf_base = av_mallocz((w + max_xblen) *
                                               h * sizeof(IDWTELEM));
    ...
    if (!(s->sbsplit  = av_malloc(sbwidth * sbheight))                  ||
        !(s->blmotion = av_malloc(sbwidth * sbheight * 4 *
                                  sizeof(*s->blmotion)))                ||
        !(s->edge_emu_buffer_base = av_malloc((w + 64) *
                                              DIRAC_MAX_BLOCKSIZE))     ||
        !(s->mctmp = av_malloc((w + 64 + DIRAC_MAX_BLOCKSIZE) *
                               h * DIRAC_MAX_BLOCKSIZE *
                               sizeof(*s->mctmp)))                      ||
        !(s->mcscratch = av_malloc((w + 64) * DIRAC_MAX_BLOCKSIZE))) {
        free_sequence_buffers(s);
        return AVERROR(ENOMEM);
    }
...
static av_cold int dirac_decode_init(AVCodecContext *avctx)
{
    ...
    ff_diracdsp_init(&s->diracdsp, avctx);
...
static inline void coeff_unpack_arith(DiracArith *c, int qfactor, int qoffset,
                                      DiracSubBand *b, IDWTELEM *buf, int x,
                                      int y)
{
    int coeff, sign;
    int sign_pred = 0;
    ...
    /* Determine if the pixel has only zeros in its neighborhood */
    if (x) {
...
static inline void codeblock(DiracContext *s, DiracSubBand *b,
                             ...
                             int blockcnt_one, int is_arith)
{
    ...
    b->quant = FFMIN(b->quant, DIRAC_MAX_QUANT);
...
static inline void intra_dc_prediction(DiracSubBand *b)
{
    IDWTELEM *buf = b->ibuf;
    int x, y;
...
static void decode_component(DiracContext *s, int comp)
{
    ...
    DiracSubBand *bands[3 * DIRAC_MAX_DWT_LEVELS + 1];
    ...
    for (level = 0; level < s->wavelet_depth; level++) {
        for (orientation = !!level; orientation < 4; orientation++) {
            DiracSubBand *b = &s->plane[comp].band[level][orientation];
            bands[num_bands++] = b;
            ...
            if (b->length) {
                ...
                b->length     = FFMIN(b->length,
                                      FFMAX(get_bits_left(&s->gb) / 8, 0));
                skip_bits_long(&s->gb, b->length * 8);
            }
        }
        /* arithmetic coding has inter-level dependencies,
         * so we can only execute one level at a time */
        if (s->is_arith)
            avctx->execute(avctx, decode_subband_arith,
                           &s->plane[comp].band[level][!!level],
                           NULL, 4 - !!level, sizeof(DiracSubBand));
    }
    /* golomb coding has no inter-level dependencies,
     * so we can execute all subbands in parallel */
    if (!s->is_arith)
...
static void lowdelay_subband(DiracContext *s, GetBitContext *gb, int quant,
                             ...
                             DiracSubBand *b1, DiracSubBand *b2)
{
    ...
    int qfactor = qscale_tab[FFMIN(quant, DIRAC_MAX_QUANT)];
    int qoffset = qoffset_intra_tab[FFMIN(quant, DIRAC_MAX_QUANT)];
...
static int decode_lowdelay(DiracContext *s)
{
    ...
    slices = av_mallocz(s->lowdelay.num_x * s->lowdelay.num_y *
                        sizeof(*slices));
    if (!slices) {
        free_sequence_buffers(s);
        return AVERROR(ENOMEM);
    ...
    avctx->execute(avctx, decode_lowdelay_slice, slices, NULL, slice_num,
                   sizeof(*slices));
    /* [DIRAC_STD] 13.3 intra_dc_prediction() */
...
static void init_planes(DiracContext *s)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        for (level = s->wavelet_depth - 1; level >= 0; level--) {
            ...
            h = h >> 1;
            for (orientation = !!level; orientation < 4; orientation++) {
                DiracSubBand *b = &p->band[level][orientation];
...
static int dirac_unpack_prediction_parameters(DiracContext *s)
{
    ...
    if (s->plane[0].xbsep < s->plane[0].xblen / 2 ||
...
#define CHECKEDREAD(dst, cond, errmsg)                                  \
    ...
    CHECKEDREAD(s->wavelet_depth, tmp > DIRAC_MAX_DWT_LEVELS || tmp < 1,
    ...
    if (!s->low_delay) {
        ...
        if (get_bits1(gb)) {
            for (i = 0; i <= s->wavelet_depth; i++) {
                CHECKEDREAD(s->codeblock[i].width, tmp < 1,
                            "codeblock width invalid\n");
                CHECKEDREAD(s->codeblock[i].height, tmp < 1,
                                  ...
                                  int refmask)
{
    ...
    /* return the majority */
    pred = (block[-1].ref          & refmask) +
           (block[-stride].ref     & refmask) +
                                ...
                                DiracBlock *block, int stride, int x, int y)
{
    ...
    if (!block->ref) {
        ...
        for (i = 0; i < 3; i++)
            block->u.dc[i] += dirac_get_arith_int(arith + 1 + i,
                                                  CTX_DC_F1, CTX_DC_DATA);
        return;
    }

    if (s->globalmc_flag) {
        block->ref |= pred_block_mode(block, stride, x, y,
...
static int dirac_unpack_block_motion_data(DiracContext *s)
{
    ...
    for (y = 0; y < s->sbheight; y++) {
        for (x = 0; x < s->sbwidth; x++) {
            unsigned int split = dirac_get_arith_uint(arith, CTX_SB_F1,
                                                      CTX_SB_DATA);
            if (split > 2)
                return AVERROR_INVALIDDATA;
    ...
    /* [DIRAC_STD] 12.3.4 Block motion vector elements.
     * vector_elements(ref, dirn) */
    for (y = 0; y < s->sbheight; y++)
        for (x = 0; x < s->sbwidth; x++) {
...
static void init_obmc_weight_row(DiracPlane *p, uint8_t *obmc_weight,
                                 int stride, int left, int right, int wy)
{
    int x;
...
static void init_obmc_weight(DiracPlane *p, uint8_t *obmc_weight, int stride,
                             int left, int right, int top, int bottom)
{
    int y;
...
static void init_obmc_weights(DiracContext *s, DiracPlane *p, int by)
{
    int top    = !by;
    int bottom = by == s->blheight - 1;

    /* don't bother re-initing for rows 2 to blheight - 2,
     * the weights don't change */
    if (top || bottom || by == 1) {
        init_obmc_weight(p, s->obmc_weight[0], DIRAC_MAX_BLOCKSIZE, 1, 0, top,
                         bottom);
        init_obmc_weight(p, s->obmc_weight[1], DIRAC_MAX_BLOCKSIZE, 0, 0, top,
                         bottom);
        init_obmc_weight(p, s->obmc_weight[2], DIRAC_MAX_BLOCKSIZE, 0, 1, top,
                         bottom);
    }
}
                   ...
                   uint8_t *obmc_weight, int xblen, int yblen)
{
    ...
    for (y = 0; y < yblen; y++) {
        ...
        dst         += stride;
        obmc_weight += DIRAC_MAX_BLOCKSIZE;
    }
}
                     ...
                     int plane, int dstx, int dsty)
{
    ...
    case 2:
        ...
        s->put_pixels_tab[idx](&s->diracdsp, s->mcscratch, src, p->stride,
                               p->yblen);
        if (s->weight_func)
    ...
    case 3:
        ...
        if (s->biweight_func) {
            ...
            s->put_pixels_tab[idx](&s->diracdsp, s->mcscratch + 32, src,
                                   p->stride, p->yblen);
            s->biweight_func(s->mcscratch, s->mcscratch + 32, p->stride,
                             s->weight_log2denom, s->weight[0], s->weight[1],
                             p->yblen);
        } else
            s->avg_pixels_tab[idx](&s->diracdsp, s->mcscratch, src, p->stride,
                                   p->yblen);
        break;
    }
                                 ...
                                 int width, int height)
{
    ...
    s->diracdsp.dsp.draw_edges(ref->hpel[plane][0],
    ...
    if (!ref->interpolated[plane]) {
        s->diracdsp.dirac_hpel_filter(ref->hpel[plane][1], ref->hpel[plane][2],
                                      ...
                                      ref->avframe.linesize[plane], width,
                                      height);
        s->diracdsp.dsp.draw_edges(ref->hpel[plane][1],
                                   ref->avframe.linesize[plane], width, height,
                                   edge, edge, EDGE_TOP | EDGE_BOTTOM);
        s->diracdsp.dsp.draw_edges(ref->hpel[plane][2],
                                   ref->avframe.linesize[plane], width, height,
                                   edge, edge, EDGE_TOP | EDGE_BOTTOM);
        s->diracdsp.dsp.draw_edges(ref->hpel[plane][3],
                                   ref->avframe.linesize[plane], width, height,
                                   edge, edge, EDGE_TOP | EDGE_BOTTOM);
    }
...
{
    DiracDWTContext d = { 0 };
    int y, i, comp, dsty, ret;

    if (s->low_delay) {
        /* [DIRAC_STD] 13.5.1 low_delay_transform_data() */
        for (comp = 0; comp < 3; comp++) {
            DiracPlane *p = &s->plane[comp];
            memset(p->idwt_buf, 0, p->idwt_stride * p->idwt_height *
                   sizeof(*p->idwt_buf));
        }
        if (!s->zero_res)
            if (ret = decode_lowdelay(s))
                return ret;
    }
    ...
    for (comp = 0; comp < 3; comp++) {
        ...
        if (ret = ff_spatial_idwt_init(&d, p->idwt_buf, p->idwt_width,
                                        p->idwt_height, p->idwt_stride,
                                        s->wavelet_idx + 2, s->wavelet_depth,
                                        p->idwt_tmp)) {
            av_log(s, AV_LOG_ERROR, "Unable to init spatial idwt\n");
            return ret;
        }

        if (!s->num_refs) { /* intra */
            for (y = 0; y < p->height; y += 16) {
                ff_spatial_idwt_slice(&d, y + 16); /* decode */
            ...
            for (i = 0; i < s->num_refs; i++)
                if (ret = interpolate_refplane(s, s->ref_pics[i], comp,
                                               p->width, p->height))
                    return ret;
            ...
            for (y = 0; y < s->blheight; y++) {
                ...
                ff_spatial_idwt_slice(&d, start + h); /* decode */
                s->diracdsp.add_rect_clamped(frame + start * p->stride, mctmp,
                                             p->stride,
                                             p->idwt_buf + start * p->idwt_stride,
                                             p->idwt_stride, p->width, h);
...
static int dirac_decode_picture_header(DiracContext *s)
{
    ...
    for (i = 0; i < s->num_refs; i++) {
        ...
        /* This is needed if the referenced picture hasn't yet arrived */
        for (j = 0; j < DIRAC_MAX_REFERENCE_FRAMES && refdist; j++)
            if (s->ref_frames[j] &&
                (distance = FFABS(s->ref_frames[j]->avframe.display_picture_number -
                                  refnum)) < refdist) {
    ...
    if (s->current_picture->avframe.reference) {
        ...
        while (add_frame(s->ref_frames, DIRAC_MAX_REFERENCE_FRAMES,
                         s->current_picture)) {
            DiracFrame *removed_frame;
            int disp_picnum = s->ref_frames[0]->avframe.display_picture_number;
            av_log(s->avctx, AV_LOG_ERROR, "Reference frame overflow\n");
            removed_frame = remove_frame(s->ref_frames, disp_picnum);
            removed_frame->avframe.reference &= DELAYED_PIC_REF;
        }
    }

    if (s->num_refs) {
        /* [DIRAC_STD] 11.2 Picture Prediction Data. picture_prediction() */
        if (ret = dirac_unpack_prediction_parameters(s))
            return ret;
        /* [DIRAC_STD] 12. Block motion data syntax */
        if (ret = dirac_unpack_block_motion_data(s))
            return ret;
    }
    /* [DIRAC_STD] 11.3 Wavelet transform data */
    if (ret = dirac_unpack_idwt_params(s))
        return ret;
                                  ...
                                  int size)
{
    ...
    if (parse_code == pc_seq_header) {
        ...
        if (ret = avpriv_dirac_parse_sequence_header(avctx, &s->gb,
                                                     &s->source))
            return ret;
        ...
        if (ret = alloc_sequence_buffers(s))
            return ret;
        ...
        /* [DIRAC_STD] 13.0 Transform data syntax. transform_data() */
        if (ret = dirac_decode_frame_internal(s))
            return ret;
    }
    return 0;
}
                              ...
                              int *data_size, AVPacket *pkt)
{
    ...
    *data_size         = 0;
    ...
    for (;;) {
        ...
        if (ret = dirac_decode_data_unit(avctx, buf + buf_idx, data_unit_size)) {
    ...
    if (s->current_picture->avframe.display_picture_number > s->frame_number) {
        ...
        if (add_frame(s->delay_frames, DIRAC_MAX_DELAY, s->current_picture)) {
            int min_num = s->delay_frames[0]->avframe.display_picture_number;
            /* Too many delayed frames, so we display the frame with the
             * lowest pts */
            av_log(avctx, AV_LOG_ERROR,
                   "Too many delayed frames. Duplicate last frame.\n");
            ...
            add_frame(s->delay_frames, DIRAC_MAX_DELAY, s->current_picture);
        }
    ...
    } else if (s->current_picture->avframe.display_picture_number ==
               s->frame_number) {
